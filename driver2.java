...
public static void primitivePriorityScheduling(double j){
   

    if(Queue1.isEmpty())    return;
    else {
    for(PCB RR : Queue1) {
    	 boolean flagArrival = true ;

    		 
    	for(PCB SJF : Queue2) {
    		
    		if(RR.getArrivalTime() <= j) {
    			flagArrival = false ;
...
public static void primitivePriorityRRScheduling(double j){
...
public static void RRScheduling() {
    ...
    // Find the last process in the cpuExecutionQueue to assign its Termination time to the current time
    ...
    // Call the method to prioritize processes in Queue1 before starting the scheduling
    primitivePriorityRRScheduling(currentTime);
    ...
    while (!Queue1.isEmpty()) {
        // Check if there are processes arriving in Queue2 before the current time
        for (PCB process : Queue2) {
            if (process.getArrivalTime() <= currentTime) {
                primitivePriorityRRScheduling(currentTime); // Prioritize Queue1 processes
                break;
            }
        }

        // Retrieve and remove the first process from Queue1
        PCB process = Queue1.poll();

        // If the process is not in the starting time map, set its starting time
        if (!startingTimeMap.containsKey(process.getProcessID())) {
            startingTimeMap.put(process.getProcessID(), currentTime);
        }
